...
struct ActionType {
    ...
    parents: Vec<ActionTypeParent>
}
#[derive(Serialize,Clone, Deserialize)]
struct ActionTypeParent{
    id:String,
    parent:String,
    required:bool
} 
...
fn overwrite_action_types(new_action_types: Vec<ActionType>) -> Result<(), ()> {
    ...
    conn.execute("DROP TABLE IF EXISTS action_types_parents", [])
        .unwrap();
    conn.execute(
        "CREATE TABLE if not exists action_types_parents(id TEXT,parent TEXT, required INTEGER)",
        [],
    ).unwrap();
    for action in new_action_types {
        ...
        for parent in action.parents {
            conn.execute("INSERT INTO action_types_parents(id, parent) VALUES((?1),(?2))", (
                action.id.to_owned(),
                parent.id,
                parent.required as u8
            )).unwrap();
        }
    }
...
fn grab_action_types() -> Result<Vec<ActionType>, ()> {
    ...
    // });
    //"CREATE TABLE if not exists action_types_parents(id TEXT,parent TEXT)",
    let query = format!("SELECT * FROM action_types_parents");
    let mut stmt = conn.prepare(&query).unwrap();
    let mut rows = stmt.query([]).unwrap();
    let mut parents:HashMap<String,Vec<ActionTypeParent>> = HashMap::new();
    while let Some(row) = rows.next().unwrap() {
        let current_parent:ActionTypeParent = ActionTypeParent{ id:row.get(0).unwrap(), parent: row.get(1).unwrap(), required:row.get(2).unwrap() };
        if parents.contains_key(&current_parent.id){
            let old_parents: &mut Vec<ActionTypeParent> = parents.get_mut(&current_parent.id).unwrap();
            old_parents.push(current_parent);

        }else{
            parents.insert(current_parent.id.clone(),vec![current_parent]);
        }
    }
    ...
    while let Some(row) = rows.next().unwrap() {
        ...
        if action_type_len > 0 && action_types[action_type_len - 1].id == current_id {
            ...
            let mut current_parents:Vec<ActionTypeParent>  = Vec::new();
            if parents.contains_key(&current_id){
                current_parents = parents.get(&current_id).unwrap().to_vec();
            }
